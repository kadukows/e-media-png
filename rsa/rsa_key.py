...
def encrypt_CBC(b: bytes, public_key: RsaPublicKey) -> EncryptedBytes:
    ...
    for offset in progressbar.progressbar(range(0, range_to_encrypt, decrypted_block_size)):
        ...
        xor_bytes_to_encrypt = bytes(bytes_to_xor ^ vector for (bytes_to_xor, vector) in zip(bytes_to_encrypt, vector[0:len(bytes_to_encrypt)]))
    ...
    xor_bytes_to_encrypt = bytes(b ^ vector for (b, vector) in zip(last_bytes_to_encrypt, vector[0:len(last_bytes_to_encrypt)]))
...
def decrypt_CBC(enc_bytes: EncryptedBytes, private_key: RsaPrivateKey) -> bytes:
    ...
    for main_bytes_offset in progressbar.progressbar(range(0, range_to_decrypt, decrypted_block_size)):
        ...
        xor_bytes_decrypted = bytes(b ^ vector for (b, vector) in zip(decrypted, vector[0:len(to_decrypt_bytes)]))
        vector = to_decrypt_bytes
    ...
    xor_bytes_decrypted = bytes(b ^ vector for (b, vector) in zip(last_decrypted, vector[0:len(last_to_decrypt_bytes)]))
